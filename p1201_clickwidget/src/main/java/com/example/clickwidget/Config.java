...
public class Config extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int cnt = sp.getInt(Config.WIDGET_COUNT + widgetID, -1);
        if (cnt == -1) {
            SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss");
            String currentTime = sdf.format(new Date(System.currentTimeMillis()));

            SharedPreferences.Editor editor = sp.edit();
            editor.putInt(WIDGET_COUNT + widgetID, 0);
            editor.putString(WIDGET_TIME + widgetID, currentTime);
            editor.commit();
        }
    }
    ...
    public void onClick(View v){
        ...
        Widget.updateWidget(this, AppWidgetManager.getInstance(this), widgetID);
